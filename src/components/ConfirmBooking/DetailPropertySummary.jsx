...
const DetailPropertySummary = ({ property }) => {
  const amenities = [
    ...
    { icon: '🛁', label: `${property.baths} Baths` },
    { icon: '📐', label: `${property.sqft} sqft` },
  ]
  return (
    <div className='rounded-xl border border-gray-200 overflow-hidden bg-white'>
      ...
      <div className='flex items-start justify-between px-5 pt-4 pb-2'>
        ...
        <div className='text-right'>
          <span className='text-xl font-semibold text-blue-600'>${property.price.toLocaleString()}</span>
          <span className='text-xs text-gray-400 block'>/mo</span>
        </div>
      </div>
      <div className='flex gap-2 px-5 pb-5'>
        {amenities.map((a) => (
          <div key={a.label} className='flex-1 bg-gray-50 border border-gray-100 rounded-lg py-2 px-3 text-center'>
