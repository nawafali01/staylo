...
const PropertyCards = () => {
  ...
  const properties = [
    { 
      ...
      stats: [
        { label: "3 Bed", icon: InboxStackIcon },
    ...
    { 
      ...
      stats: [
        { label: "2 Bed", icon: InboxStackIcon },
        { label: "2 Bath", icon: SparklesIcon },
    ...
    { 
      ...
      stats: [
        { label: "4 Bed", icon: InboxStackIcon },
  ...
  return (
    <section className="p-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {properties.map((p) => (
          /* Yahan 'group' class lagai hai taakay card hover par andar ki cheezain trigger hon */
          <article key={p.id} className="group border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 bg-white">
            ...
            <div className="relative overflow-hidden cursor-pointer">
              ...
              {/* Status Badge: Hover par show hoga aur thoda slide karega */}
              <div className={`absolute top-4 left-4 z-20 transition-all duration-300 opacity-0 group-hover:opacity-100 translate-y-[-10px] group-hover:translate-y-0
                ...
                ${p.status === "Available" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}>
              ...
              <footer className="flex justify-between text-gray-600">
                {p.stats.map((stat, index) => (
                  <div key={index} className="flex flex-col items-center gap-1">
                    <stat.icon className="h-5 w-5 text-gray-400 group-hover:text-blue-500 transition-colors" />
