...
const SearchField = ({ icon: Icon, label, value, placeholder, options, isInput, onChange, onSelect, isOpen, toggleDropdown, isLast }) => (
      ...
      <div className="absolute top-full left-0 mt-3 w-56 bg-white shadow-2xl rounded-xl border border-gray-100 py-2 z-50">
        {options.map((item) => (
          <div 
            ...
            onClick={(e) => {
              e.stopPropagation(); // Taake main div ka click trigger na ho
              onSelect(item);
            }}
          >
            {item}
